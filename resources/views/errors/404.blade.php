...
<head>
    ...
    @endif
    <title>{{ $site_settings['site_name'] ?? '404 - Sistem Absensi SMKN Tengaran' }}</title>
...
<body class="bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 antialiased min-h-screen flex flex-col justify-center relative overflow-hidden transition-colors duration-300">
    ...
    <main class="relative z-10 px-4 w-full flex justify-center">
        <div class="glass-card max-w-lg w-full p-10 md:p-14 text-center rounded-[2rem] shadow-2xl transition-all duration-300">
            <div class="text-9xl font-extrabold text-transparent bg-clip-text bg-gradient-to-br from-emerald-500 to-emerald-700 dark:from-emerald-400 dark:to-emerald-600 mb-6 drop-shadow-sm">
                404
            </div>
            
            <h1 class="text-3xl font-bold text-slate-800 dark:text-white mb-4 transition-colors">Halaman Tidak Ditemukan</h1>
            <p class="text-slate-500 dark:text-slate-400 text-base leading-relaxed mb-10 transition-colors">
                halaman yang Anda tuju tidak dapat ditemukan. Halaman tersebut mungkin telah dipindahkan, dihapus, atau Anda memasukkan URL yang salah.
            </p>
